...
Let's get it set up. The dev environment already installed cert-manager (which the controller needs for its webhook TLS). Install the CRDs and controller:
...
Open `src/workflows.py` and update the `ResearchPollingWorkflow`. Add a few new fields to track draining state and wake-up signals:
