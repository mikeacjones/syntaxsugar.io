...
Build and deploy, then start the polling workflow:
...
The polling workflow is running on `v3-versioned`. Now deploy a new version:

```bash
./scripts/build-and-deploy.sh v4-versioned --versioned
```

Check the Deployments tab in the Temporal UI. You'll see `v3-versioned` is in the Draining state because the polling workflow is still pinned to it. Now send it some work:
...
Deploy and start the polling workflow again with the updated code:
...
```bash
./scripts/build-and-deploy.sh v6-versioned --versioned
```

Check the Deployments tab. The polling workflow is still on `v5-versioned`. Now send it a research request:
...
This time the workflow wakes up, sees the version change, drains, and continues-as-new onto `v6-versioned`. Check the Deployments tab again. The workflow has migrated. The V5 deployment should transition to Drained.

{/* TODO: Screenshot showing the continue-as-new chain from V5 to V6 in the Temporal UI */}
