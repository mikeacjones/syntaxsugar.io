...
Product wants a fact-check step before synthesis. Totally reasonable. Let's add it.

Open `src/workflows.py` and add the `fact_check` activity call right before the signal gate. First, update the import at the top of the file to include `fact_check`:
...
Then add the `fact_check` call right before `await workflow.wait_condition(lambda: self._proceed)`:

```python
        findings.append(result)

    # Fact-check before proceeding to synthesis  # [!code ++]
    checked = await workflow.execute_activity(    # [!code ++]
        fact_check,                               # [!code ++]
        args=[topic, findings],                   # [!code ++]
...
What happens: the V2 worker picks up the workflow task and replays the history. Plan matches. Research steps match. Then V2's code says "schedule fact_check" but the V1 history has no fact_check at this point. It went straight to the signal wait. The commands don't match the events. Non-determinism error.
...
```python
from temporalio.worker.deployment_config import (  # [!code ++]
    WorkerDeploymentConfig,                         # [!code ++]
    WorkerDeploymentVersion,                        # [!code ++]
)                                                   # [!code ++]
```
...
Now tell Temporal that `ResearchWorkflow` should stay on whatever version started it. Add the import and update the decorator:

```python
from temporalio.workflow import VersioningBehavior               # [!code ++]

@workflow.defn(versioning_behavior=VersioningBehavior.PINNED)    # [!code ++]
@workflow.defn                                                   # [!code --]
class ResearchWorkflow:
    # ... rest of the code stays the same
```

`PINNED` means: once a workflow starts on a particular build ID, all of its tasks get routed exclusively to workers running that same build. The code never changes underneath a running workflow.
...
Start a workflow, let the research steps complete, then deploy V2 mid-flight:
...
Check `kubectl get pods`. You'll see pods for both versions running simultaneously. Now signal the V1 workflow:
...
This time the V1 workflow finishes on the V1 pod. No errors. New workflows start on V2 with the fact-check step. No lost work.
...
The `ResearchPollingWorkflow` in the starter code runs indefinitely, processing research requests as they arrive via signals. Since it's pinned, it'll stay on V1 forever. That keeps old worker pods alive indefinitely, which isn't what we want.

The solution: detect when a new version is deployed and gracefully migrate using continue-as-new.

### Detect the Version Change

Temporal provides `workflow.info().is_target_worker_deployment_version_changed()`. This returns `True` when the server has set a newer "current" version for this deployment. We can check it in our wait condition.

Open `src/workflows.py` and update the `ResearchPollingWorkflow`. First, add the PINNED decorator and a few new fields:
...
Add a `wake_up` signal (we'll need this in the next section):
...
Now update the main loop. Replace the `wait_condition` and add the version check:
...
```python
    ...
    async def run(self, state: PollingState) -> None:
        ...
        while True:
            ...
            # Version change or history getting large: drain and migrate
            if (                                                                      # [!code ++]
...
Let's unpack what's happening in that version-check block:
...
3. We call `continue_as_new` with `initial_versioning_behavior=AUTO_UPGRADE`. This tells Temporal to start the new execution on the latest current version, not on the same build ID we started on.

The state carries over. Pending requests and the completed count survive the migration. From the outside, it looks like the workflow just kept running. But under the hood, it jumped from V1 to V2.
...
That's why we added the `wake_up` signal earlier. After deploying a new version, send a signal to nudge the workflow:
