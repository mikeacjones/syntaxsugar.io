...
2. I have the file hosted for you in Dropbox, so we're going to use a REST Request in order to download this file:
   ...
   2. Paste the URL to the file into the REST request component, and make sure the request is set as a GET operation.
      - URL: `https://www.dropbox.com/s/w2knq9ds4wlmp4o/COVID-19_Case_Surveillance_Public_Use_Data.csv?dl=1`
3. After the Request operation, add a Transform component and paste the following dataweave. By adding `deferred=true`, we tell the script to evaluate in a lazy way, meaning it won't process the transformation until the output is actually being consumed.
```data-weave
%dw 2.0
